...
OLM entry catalog
   |          |             |              |
folders    messages     attachments   contacts/calendars/notes
   |          |             |              |
   +----------+-------------+--------------+
              |
              v
...
The OLM parsers turn Outlook XML into normalized accounts, folders, messages, contacts, calendar items, notes, and attachment references. Normalized message headers include sender, To/CC/BCC, sent and received dates, message ID, read/flag state, and attachment metadata. Contact normalization retains names and nickname, company/title/department/office, manager/assistant/spouse, email addresses, phone numbers, postal addresses, websites, birthdays/anniversaries, category labels, distribution-list members, bounded image data, notes, and modification date. Calendar normalization retains a source-scoped internal identity, the original calendar UID for interoperable export, title, start/end, all-day/private flags, location, description, organizer, attendees, reminders, normalized time-zone labels, status, cancellation, series/recurrence identifiers, and absolute or relative recurrence metadata. Note normalization retains only text and available creation/modification dates.

XML parsing uses `XMLParser` event callbacks rather than constructing a DOM. Mail bodies remain lazy. `Contacts.xml`, `Calendar.xml`, and `Notes.xml` collections are cataloged from the central directory without decoding during archive opening and parsed only when selected. `Tasks.xml` is also cataloged, but is not decoded until a real schema fixture exists. A first parse reports its source, archive-read and record-parse phases, bounded decoded-byte total, recovered-record count, elapsed time, and cache-hit state; cancellation propagates into the detached parser and a partial canceled result is never cached. After a successful bounded parse, normalized records are atomically encoded into a separate binary property-list cache keyed by archive fingerprint, collection kind, and source path. A cache schema mismatch, archive size/modification change, source mismatch, decode error, or Delete Cache action causes a safe miss and reparsing; the OLM remains authoritative. Contacts and notes retain incremental list presentation. Calendar mode deliberately loads the complete selected collection—or all collections when explicitly selected—so its month, week, day, chronological range, and whole-calendar exports cannot silently omit events beyond an initial page.
...
The primary interface is a native three-column `NavigationSplitView` with a fixed bottom Mail, Calendar, Contacts, and Notes switcher. The buttons remain outside the scrolling source list, visibly identify the active mode, and provide Command-1 through Command-4 shortcuts. In Mail, indexed folder and search rows load without message-body decoding, retain their requested order, and hydrate their full message only when selected. Unindexed fallback pages retain their requested order after parallel decoding.

Contacts use a compact avatar/name list and a native-style detail card with a recovered embedded photo, initials, or a distribution-list symbol, title/company identity, grouped email/phone/address/date/category/note/organization/relationship/member panels, copy controls, and explicit export actions. Contact images are accepted only as bounded JPEG, PNG, GIF, or TIFF payloads and are capped at 10 MB after decoding. Calendar mode loads the complete selected collection before presentation. Its upper middle pane is the selected-day agenda, its lower middle pane is event detail behind a draggable divider, and its large right pane switches among a window-scaling six-week month grid, day timeline, week timeline, and globally chronological list. The list accepts an inclusive date range, presets, one or all calendar sources, and optional canceled events. It exports the exact materialized occurrences in the visible range; selecting one recurrence instance exports that instance rather than the entire series. A separately labeled whole-calendar iCalendar action ignores view/search/range state and exports either the selected calendar or all recovered calendars to one `.ics` file.

Occurrence calculation runs away from the main actor and expands recognized daily, multi-weekday weekly, absolute monthly/yearly, and relative nth/last-weekday monthly/yearly recurrence masters while honoring interval, scientific or integer occurrence-count, explicit no-end, and end-date limits. Common Outlook recurrence aliases are normalized. An exception with matching source-scoped series and recurrence identifiers replaces its generated master occurrence; a canceled exception suppresses that occurrence. Event calculations use the normalized event time zone so local wall time remains stable across daylight-saving transitions. Common Windows zone names map to IANA identifiers, UTC aliases normalize consistently, and ISO, compact, date-only, and .NET millisecond timestamps are accepted.

The three columns are:

1. Accounts/folders or contact/calendar collections
2. Filterable message/contact/note results, or calendar agenda above event detail
3. Message/contact/note detail, or the large calendar month/week/day/list workspace
...
Archive opening and indexing run in cancelable tasks. Opening reports ZIP64-directory reading, in-memory entry scanning, and Outlook cataloging phases, with entry progress and the number of bytes read for bounded ZIP structures relative to total archive size. The operations panel reports central-directory, message, attachment, duplicate-path, CRC-failure, unsupported-compression, recovered-malformed-message, unreadable-message, index-progress, combined mail/item cache-size counts, and aggregate contact/calendar/note recovery health plus detected task collections. Item diagnostics include parsed/failed collection counts, missing core fields, recovered field coverage, distribution-list membership, recurrence/cancellation/exception/time-zone counts, note counts, and detected task-family counts. Unsupported compression is known immediately from the central directory; CRC failures are recorded only when a bounded read of that entry is attempted. Rebuild clears the mail checkpoint and resumes indexing; Delete Cache removes and compacts mail rows and deletes parsed contact/calendar/note cache files.

Diagnostic-report export is an explicit local Save action. Schema version 4 contains only aggregate archive size, account/folder/message/attachment/duplicate/unreadable/recovered-malformed/CRC-failure/unsupported-compression counts, aggregate contact/calendar/note recovery metrics, detected task-collection counts, search-index progress, cache size, generation time, and privacy declarations. It never includes the archive path or filename, account/folder/source names, message/contact/calendar/note/task identifiers or content, participants, contact values, attachment names, payloads, CRC values, or cache contents.
...
- Bound any decoded contact/calendar/note collection to 512 MB and reuse its disposable normalized cache after the first successful parse.
...
8. Relative recurrence recovery, day/week calendar layouts, contact-photo/group recovery, Notes viewing/export, task-family discovery, and whole-calendar iCalendar export.
9. Verified task viewing/export after obtaining a real Tasks schema, deleted-instance recovery, remaining record families, and conversation reconstruction.
